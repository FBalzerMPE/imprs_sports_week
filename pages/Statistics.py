...
The following plot shows the current standings, both for each of the sports individually, and finally the (normalized) total score.\\
...
for i, sport in enumerate(hf.SPORTS_LIST):
    ...
    colname = f"{event.icon} ({num_done}/{num_tot}) x {event.point_weight_factor:.1f}"
    df.insert(i * 2 + 1, colname, result_perc)
score_df = df[[col for col in df.columns if " x " in col]].copy()
score_df["Total Score"] = score_df.sum(axis=1) / score_df.sum(axis=0).sum() * 500
...
chart = (
    ...
    .properties(
        title="Score distribution (note that the total scores are rescaled to sum to 500)"
    )
)
